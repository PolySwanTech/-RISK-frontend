...
export class AuthService {
  ...
  private baseAuth = environment.authServiceUrl;
  ...
  register(user: Utilisateur, rolesList: TeamMember) {
    return this.http.post<Utilisateur>(this.baseAuth + '/auth/register', user).pipe(
      switchMap(createdUser =>
        this.http.post<Utilisateur>(this.base + '/user', {
          id: createdUser.id,
          name: createdUser.username,
          teamRoleList: rolesList
        })
      )
    );
  }
  ...
  login(username: string, mdp: string) {
    return this.http.post<any>(this.baseAuth + '/auth/login', { mail: username, password: mdp })
      .subscribe({
        next: res => {
          sessionStorage.setItem('token', res.token);
          this.http.get(this.base + '/users/refresh-token', {}).subscribe(
            _ => {
